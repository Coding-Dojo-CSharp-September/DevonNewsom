                   ...
                   using(IDataReader rdr = command.ExecuteReader())
                   {
                      while(rdr.Read())
                      {
                        ...
                        for( int i = 0; i < rdr.FieldCount; i++ ) {
                            // object rdrVal;
                            // if(rdr.GetDataTypeName(i) == "DATETIME")
                            //     rdrVal = System.Convert.ToDateTime(rdr.GetValue(i));
                            // else
                            //     rdrVal = rdr.GetValue(i);
                            //object rdrVal = (rdr.GetDataTypeName(i) == "DATETIME") ? 
                                            //System.Convert.ToDateTime(rdr.GetValue(i)) : rdr.GetValue(i);
